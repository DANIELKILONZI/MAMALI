...
export default function ProductDetailClient({ initialProduct }: Readonly<{ initialProduct: Product | null }>) {
  ...
  // Server component provides the product; fetch client-side only if it
  // couldn't (e.g. transient API failure during the server render).
  useEffect(() => {
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="grid gap-10 md:grid-cols-2">
        ...
        <div>
          ...
          <button
            onClick={() => {
              const text = `${product!.name} — KSh ${price.toLocaleString('en-KE')}\n${window.location.href}`;
              window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank', 'noopener');
            }}
            className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-green-700 hover:text-green-800 hover:underline"
          >
